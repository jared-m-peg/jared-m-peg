...
Click on Edit
Click on New
And type: %ANT_HOME%\bin
Click OK on the Environment Variables window
Click OK on System Properties

Check whether Ant is configured correctly:
Open a cmd.exe window and run this command:
ant -version
It should print:
Apache Ant(TM) version 1.10.17 compiled on April 6 2026

Open another cmd.exe window and run this command:
git --version
It should print:
...
In the cmd.exe window, run:
ant dist
It should print: BUILD SUCCESSFUL
...
In a cmd.exe window run this command:
```cmd
cd C:\Games\rscplus
```
Run:
...
Right-click on white space, click on New, click on Text Document
